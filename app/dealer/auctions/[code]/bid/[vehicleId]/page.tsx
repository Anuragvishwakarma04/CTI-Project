...
import { ArrowLeft, Car, Gavel, Send, Clock, User, ChevronDown } from 'lucide-react';
...
export default function BiddingPage() {
  ...
  const handleBidSubmit = async () => {
    ...
    try {
      ...
      const token = auth.getToken();
      if (!token) { router.push('/login'); return; }
      const res = await api.placeBid(code as string, vehicleId as string, amount, token);
      if (res.success) {
        setBidAmount('');
        await fetchBids();
      } else {
  ...
  if (!vehicle || !auction) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-12 text-center">
        ...
        <button onClick={() => router.back()} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm">Go Back</button>
      </div>
    );
  }

  const startingBid = bids?.length > 0 ? Number(bids[bids.length - 1].bid_amount) : 0
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center px-4 py-6">
      ...
      <div className="w-full max-w-6xl bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
        ...
        <div className="max-w-6xl mx-auto px-4 py-5">
          ...
          {/* Back Button */}
          <button
            onClick={() => router.push(`/dealer/auctions/${code}`)}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-5 items-start">
            ...
            <div className="space-y-3">
                ...
                <div className="flex gap-2 overflow-x-auto pb-1">
                  {images.map((img: string, i: number) => (
                    <button
                      ...
                      className={`flex-shrink-0 w-20 h-14 rounded-lg overflow-hidden border-2 transition-all ${activeImg === i ? 'border-yellow-500' : 'border-transparent'
                        }`}
                    >
              ...
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                {specsOpen && (
                  <div className="px-4 pb-4 grid grid-cols-2 gap-3 border-t border-gray-100">
                    ...
                    {vehicle.model && <SpecItem label="Model" value={`${vehicle.model} ${vehicle.variant || ''}`} />}
                    {vehicle.year && <SpecItem label="Year" value={vehicle.year} />}
                    {vehicle.fuel_type && <SpecItem label="Fuel" value={vehicle.fuel_type} />}
                    {vehicle.km_driven && <SpecItem label="KMs Run" value={Number(vehicle.km_driven).toLocaleString('en-IN')} />}
                    {vehicle.transmission && <SpecItem label="Transmission" value={vehicle.transmission} />}
                    {/* {vehicle.engine_cc && <SpecItem label="Engine" value={`${vehicle.engine_cc} cc`} />} */}
                    {vehicle.color && <SpecItem label="Colour" value={vehicle.color} />}
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                >
                  ...
                  <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${addInfoOpen ? 'rotate-180' : ''}`} />
                </button>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
                <p className="text-xs font-bold text-gray-400 tracking-widest uppercase mb-1">Auction Ends In</p>
                <p className={`font-mono text-5xl font-bold tracking-wider ${isLive ? 'text-red-500' : 'text-gray-400'}`}>
                  {timeRemaining}
                </p>
              </div>
              ...
              {/* Current Highest Bid */}
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Current Highest Bid
                  <span className="text-green-500 font-normal  ml-1 capitalize ">({highestBid ? ` ${highestBid.dealer.name || highestBid.dealer.business_name}` : 'No bids yet'})</span>
                </p>
                <p className="text-4xl font-bold text-gray-900 font-mono">
                  ₹{highestBidAmount > 0 ? highestBidAmount.toLocaleString('en-IN') : '0'}
                </p>
                <div className="flex items-center justify-between mt-2">
                  <p className="text-xs text-gray-500">
                    Participant Bids: <span className="font-bold text-gray-800">{totalBids}</span>
                  </p>
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-2 min-h-[140px] max-h-[220px] bg-gray-50/50">
                  {bids.length === 0 ? (
                    ...
                    bids.map((bid, i) => {
                      ...
                      return (
                        <div key={bid.id || i} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                          <div className="max-w-[80%]">
                            <div className={`rounded-2xl px-3 py-2 ${isMine
                              ? 'bg-blue-900 text-white rounded-br-sm'
                              : 'bg-white border border-gray-200 rounded-bl-sm'
                              } ${isHighest ? 'ring-2 ring-green-400 ring-offset-1' : ''}`}>
                              <div className="flex items-center gap-1.5 mb-0.5">
                                <User className={`w-3 h-3 ${isMine ? 'text-white/50' : 'text-gray-400'}`} />
                                <span className={`text-[10px] font-semibold ${isMine ? 'text-white/70' : 'text-gray-500'}`}>
                                  {isMine ? 'You' : bid.dealer.name}
                                </span>
                                {isHighest && (
                                  <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold ${isMine ? 'bg-white/20 text-white' : 'bg-green-100 text-green-700'
                                    }`}>HIGHEST</span>
                                )}
                              </div>
                              <p className={`text-base font-bold font-mono ${isMine ? 'text-white' : 'text-gray-900'}`}>
                            ...
                            <p className={`text-[9px] mt-1 px-1 text-gray-400 ${isMine ? 'text-right' : ''}`}>
                              {new Date(bid.bid_time).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
                            </p>
                          </div>
                        </div>
                      );
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1 relative">
                          ...
                          <input
                            ...
                            onKeyDown={e => e.key === 'Enter' && !submitting && handleBidSubmit()}
                            placeholder="Enter bid amount"
                            className={`w-full pl-7 pr-3 py-2.5 border rounded-lg text-sm font-semibold outline-none transition-colors ${error ? 'border-red-400' : 'border-gray-200 focus:border-blue-400'
                              }`}
                            disabled={submitting}
                          />
                        </div>
