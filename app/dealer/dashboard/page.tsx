...
export default function DealerDashboardPage() {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/dealer/customers?page=${customersPage}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            Accept: "application/json",
  ...
  useEffect(() => {
    ...
    if (activeSection === "customers" && user) {
      fetchCustomers();
    }
  }, [
    activeSection,
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      if (data.success) {
        setDealerData(data.data.dealer);
            ...
            <div className="card p-4 sm:p-6 md:p-8">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
                <div className="relative w-20 h-20 sm:w-24 sm:h-24">
                  {dealerData?.profile_image || user.avatar ? (
                    <Image
                      src={dealerData?.profile_image || user.avatar}
                      alt={user.name}
                      fill
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-gray-500">
                  {customersPagination.total} Total
                </span>
              </div>
              ...
              {customersLoading ? (
                ...
                <div className="card p-12 text-center">
                  ...
                  <p className="text-gray-600">
                    You don't have any connected customers at the moment.
                  </p>
                </div>
              ) : (
                <>
                          ...
                          {customers.map((customer: any) => (
                            <tr key={customer.id} className="hover:bg-gray-50 transition">
                              ...
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-2 text-sm text-gray-600">
                                  <Calendar className="w-4 h-4" />
                                  {new Date(customer.connected_at).toLocaleDateString('en-IN', {
                                    day: 'numeric',
                                    month: 'short',
                                    year: 'numeric',
                                  })}
                  ...
                  {customersPagination.last_page > 1 && (
                    <div className="flex items-center justify-between bg-white rounded-lg shadow-sm px-6 py-4">
                      ...
                      <div className="flex gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          {Array.from({ length: Math.min(5, customersPagination.last_page) }, (_, i) => {
                            let pageNum;
                            if (customersPagination.last_page <= 5) {
                              pageNum = i + 1;
                            } else if (customersPage <= 3) {
                              pageNum = i + 1;
                            } else if (customersPage >= customersPagination.last_page - 2) {
                              pageNum = customersPagination.last_page - 4 + i;
                            } else {
                        ...
                        <button
                          onClick={() => setCustomersPage(prev => Math.min(customersPagination.last_page, prev + 1))}
                          disabled={customersPage === customersPagination.last_page}
                          className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition"
                        >
                          Next
                        </button>
                ...
                {showroomLoading ? (
                  ...
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {showrooms.map((s) => (
                      ...
                      >
                        {/* Image */}
                        <div className="relative h-44 w-full overflow-hidden rounded-t-2xl bg-gray-200">
                          {s.profileImage ? (
                            <img
                              src={s.profileImage}
                              alt={s.showroomName}
