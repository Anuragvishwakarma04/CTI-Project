...
import { Calendar, MapPin, IndianRupee, Car, Gauge, Fuel, Users, Edit, Trash2, ArrowLeft, CheckCircle, Eye, EyeOff, Power } from 'lucide-react';
import { api, auth } from '@/lib/api';

export default function DealerListingDetailPage() {
  ...
  useEffect(() => {
    ...
    setUser(savedUser);
    if (!savedUser || (savedUser.user_type !== 'dealer' && savedUser.user_type !== 'showroom')) {
      router.push('/login');
      return;
    }
    fetchVehicle();
  ...
  const handleToggleActive = async () => {
    ...
    try {
      const response = await api.toggleVehicleActive(token, vehicle.vehicle_id);
      if (response.success) {
        fetchVehicle();
      } else {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                {vehicle.is_active !== undefined && (
                  <span className={`px-3 py-1.5 rounded-full text-xs font-semibold ${vehicle.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {vehicle.status !== 'sold' && (
                <>
                  ...
                  <button onClick={handleToggleActive} className="btn-secondary bg-amber-600 text-white hover:bg-amber-700 flex items-center gap-2">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                {vehicle.body_type && (
                  <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition">
                    ...
                    <div>
                      <p className="text-xs text-gray-500 font-medium">Body Type</p>
                      <p className="font-bold text-gray-900">{vehicle.body_type}</p>
                    </div>
                  </div>
                )}
                {vehicle.location && vehicle.location !== 'Not Specified' && (
                  <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-gray-900">{vehicle.location}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            {(vehicle.purchase_price || vehicle.selling_price || vehicle.minimum_selling_price || vehicle.reconditioning_cost || vehicle.accessories_cost || vehicle.other_expenses) && (
              <div className="card p-6">
                <h2 className="text-xl font-bold mb-4">Financial Information</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {vehicle.purchase_price && (
                    <div className="p-4 bg-blue-50 rounded-xl">
                      <p className="text-xs text-gray-600 font-medium mb-1">Purchase Price</p>
                      <p className="text-lg font-bold text-gray-900">₹{parseFloat(vehicle.purchase_price).toLocaleString()}</p>
                    </div>
                  )}
                  {vehicle.selling_price && (
                    <div className="p-4 bg-green-50 rounded-xl">
                      <p className="text-xs text-gray-600 font-medium mb-1">Selling Price</p>
                      <p className="text-lg font-bold text-gray-900">₹{parseFloat(vehicle.selling_price).toLocaleString()}</p>
                    </div>
                  )}
                  {vehicle.minimum_selling_price && (
                    <div className="p-4 bg-orange-50 rounded-xl">
                      <p className="text-xs text-gray-600 font-medium mb-1">Minimum Price</p>
                      <p className="text-lg font-bold text-gray-900">₹{parseFloat(vehicle.minimum_selling_price).toLocaleString()}</p>
                    </div>
                  )}
                  {vehicle.reconditioning_cost && parseFloat(vehicle.reconditioning_cost) > 0 && (
                    <div className="p-4 bg-purple-50 rounded-xl">
                      <p className="text-xs text-gray-600 font-medium mb-1">Reconditioning Cost</p>
                      <p className="text-lg font-bold text-gray-900">₹{parseFloat(vehicle.reconditioning_cost).toLocaleString()}</p>
                    </div>
                  )}
                  {vehicle.accessories_cost && parseFloat(vehicle.accessories_cost) > 0 && (
                    <div className="p-4 bg-indigo-50 rounded-xl">
                      <p className="text-xs text-gray-600 font-medium mb-1">Accessories Cost</p>
                      <p className="text-lg font-bold text-gray-900">₹{parseFloat(vehicle.accessories_cost).toLocaleString()}</p>
                    </div>
                  )}
                  {vehicle.other_expenses && parseFloat(vehicle.other_expenses) > 0 && (
                    <div className="p-4 bg-pink-50 rounded-xl">
                      <p className="text-xs text-gray-600 font-medium mb-1">Other Expenses</p>
                      <p className="text-lg font-bold text-gray-900">₹{parseFloat(vehicle.other_expenses).toLocaleString()}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3">
                {vehicle.engine_number && (
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="text-sm text-gray-600 font-medium">Engine Number</span>
                    <span className="text-sm font-semibold text-gray-900">{vehicle.engine_number}</span>
                  </div>
                )}
                {vehicle.chassis_number && (
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="text-sm text-gray-600 font-medium">Chassis Number</span>
                    <span className="text-sm font-semibold text-gray-900">{vehicle.chassis_number}</span>
                  </div>
                )}
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                  <span className="text-sm text-gray-600 font-medium">Insurance</span>
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${vehicle.has_insurance ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    {vehicle.has_insurance ? 'Available' : 'Not Available'}
                  </span>
                </div>
                {vehicle.insurance_expiry && (
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="text-sm text-gray-600 font-medium">Insurance Expiry</span>
                    <span className="text-sm font-semibold text-gray-900">{new Date(vehicle.insurance_expiry).toLocaleDateString()}</span>
                  </div>
                )}
                {vehicle.rc_document_url && (
                  <a href={vehicle.rc_document_url} target="_blank" rel="noopener noreferrer" className="block p-3 bg-blue-50 rounded-lg hover:bg-blue-100 transition">
                    <span className="text-sm text-blue-600 font-medium">📄 View RC Document</span>
                  </a>
                )}
                {vehicle.insurance_document_url && (
                  <a href={vehicle.insurance_document_url} target="_blank" rel="noopener noreferrer" className="block p-3 bg-blue-50 rounded-lg hover:bg-blue-100 transition">
                    <span className="text-sm text-blue-600 font-medium">📄 View Insurance Document</span>
                  </a>
                )}
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {vehicle.sold_price && (
                    <div className="p-4 bg-white rounded-xl">
                      <p className="text-xs text-gray-600 font-medium mb-1">Sold Price</p>
                      <p className="text-2xl font-bold text-gray-900">₹{parseFloat(vehicle.sold_price).toLocaleString()}</p>
                    </div>
                  )}
                  {vehicle.sold_to && (
                    <div className="p-4 bg-white rounded-xl">
                      <p className="text-xs text-gray-600 font-medium mb-1">Sold To</p>
                      <p className="text-lg font-bold text-gray-900">{vehicle.sold_to}</p>
                    </div>
                  )}
                  <div className="p-4 bg-white rounded-xl col-span-2">
                    <p className="text-xs text-gray-600 font-medium mb-1">Sold Date</p>
                    <p className="text-lg font-bold text-gray-900">{new Date(vehicle.sold_at).toLocaleString()}</p>
                  </div>
                  {vehicle.sold_notes && (
                    <div className="p-4 bg-white rounded-xl col-span-2">
                      <p className="text-xs text-gray-600 font-medium mb-1">Notes</p>
                      <p className="text-sm text-gray-900">{vehicle.sold_notes}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="card p-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    ...
                    <span className="text-sm font-semibold text-gray-900">{vehicle.seller.name}</span>
                  </div>
                  {vehicle.seller.business_name && (
                    <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      <span className="text-sm text-gray-600 font-medium">Business Name</span>
                      <span className="text-sm font-semibold text-gray-900">{vehicle.seller.business_name}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="text-sm text-gray-600 font-medium">Phone</span>
                    <span className="text-sm font-semibold text-gray-900">{vehicle.seller.phone}</span>
                  </div>
                  {vehicle.seller.email && (
                    <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      <span className="text-sm text-gray-600 font-medium">Email</span>
                      <span className="text-sm font-semibold text-gray-900">{vehicle.seller.email}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="text-sm text-gray-600 font-medium">User Type</span>
                    <span className="px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">
                      {vehicle.seller.user_type.toUpperCase()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {vehicle.is_active !== undefined && (
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <span className={`px-3 py-1.5 rounded-full text-xs font-bold ${vehicle.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                      {vehicle.is_active ? 'ACTIVE' : 'INACTIVE'}
                    </span>
                  </div>
                )}
              ...
              <div className="mt-6 pt-6 border-t space-y-3">
                ...
                {vehicle.status !== 'sold' && (
                  <>
                    ...
                    <button onClick={handleToggleActive} className="w-full bg-amber-600 text-white hover:bg-amber-700 rounded-lg font-semibold flex items-center justify-center gap-2 py-3 transition">
                      {vehicle.is_active ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                      {vehicle.is_active ? 'Deactivate Listing' : 'Activate Listing'}
                    </button>
                  </>
                )}
